...
class LexEngine {
	...
	**/
	public static function parse(pattern:String):Pattern {
		var p = parseInner(ByteData.ofString(pattern));
		if (p == null)
	...
	static function parseInner(pattern:ByteData, i:Int = 0, pDepth:Int = 0):{pattern:Pattern, pos:Int} {
		function readChar() {
		...
		if (pDepth != 0)
			throw 'Found unclosed parenthesis while parsing "$pattern"';
